...
def home():
    ...
    return render_template(
        "display.html",
        table=entry_record.to_html(
            classes="table table-striped", columns=["Name", "Timestamp"], index=False
        ),
        images=images,
    )
...
def update_data():
    ...
    if file:
        ...
        if record is not None:
            ...
            if (
                last_timestamp + timedelta(minutes=10) <= record["Timestamp"][0]
                or filtered_records.empty
            ):
