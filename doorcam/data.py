...
def get_image_paths(config: dict, progressbar: bool=True, partition: Literal["train", "test"]="train") -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns all_img_paths = []
    all_img_ids = []
    ids_paths = glob(config["training"]["vggface_dir"] + "/data/train/*")
    for idx, id in tqdm(enumerate(ids_paths), total=len(ids_paths), disable=not progressbar):
        img_paths = np.array(glob(id + '/*.jpg'))
        ids = np.zeros(img_paths.shape[0], dtype=int) + idx
        all_img_paths.append(img_paths)
        all_img_ids.append(ids.astype(int))
